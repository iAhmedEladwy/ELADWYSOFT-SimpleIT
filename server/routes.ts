  ...
  app.post("/api/assets/import", authenticateUser, hasAccess(3), upload.single('file'), async (req, res) => {
    try {
      ...
      for (const row of parsedData) {
        try {
          const assetData: any = {
            // assetId is auto-generated by database, do not include it
          ...
          if (assetData.purchaseDate && assetData.purchaseDate !== '') {
            ...
            assetData.purchaseDate = !isNaN(date.getTime()) ? date.toISOString().split('T')[0] : null;
          }
          
          if (assetData.warrantyExpiryDate && assetData.warrantyExpiryDate !== '') {
            const date = new Date(assetData.warrantyExpiryDate);
            assetData.warrantyExpiryDate = !isNaN(date.getTime()) ? date.toISOString().split('T')[0] : null;
          }
