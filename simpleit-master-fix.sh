...
# SimpleIT Master Fix - Complete Solution
# This script consolidates all fixes for WebSocket, 502 errors, and authentication issues
# Run as root on your Ubuntu server to fix deployment issues
...
status_message "Starting SimpleIT Master Fix"

# Check if installation directory exists
if [ ! -d "$INSTALL_DIR" ]; then
  echo "Error: Installation directory $INSTALL_DIR not found."
  echo "This script is meant to be run on an existing SimpleIT installation."
  exit 1
fi

# FIX 1: Replace WebSocket database connection with direct PostgreSQL
status_message "Fixing database connection (WebSocket issues)"
...
# FIX 2: Install required PostgreSQL packages
status_message "Installing PostgreSQL client dependencies"
cd "$INSTALL_DIR"
su - $APP_USER -c "cd $INSTALL_DIR && npm install pg @types/pg"

# FIX 3: Update session configuration for proper authentication
status_message "Fixing session and authentication issues"

# Check if routes.ts exists and modify session settings
if [ -f "$INSTALL_DIR/server/routes.ts" ]; then
  # Create a backup
  cp "$INSTALL_DIR/server/routes.ts" "$INSTALL_DIR/server/routes.ts.bak"
  
  # Fix session configuration (replace false with true for session persistence)
...
# FIX 4: Set trust proxy in Express for proper IP handling
...
# FIX 5: Configure proper environment settings
...
# FIX 6: Update systemd service configuration
status_message "Updating systemd service configuration"
...
# FIX 7: Update Nginx configuration for proper cookie and proxy handling
status_message "Optimizing Nginx configuration"
if [ -f "/etc/nginx/sites-available/simpleit" ]; then
  # Create a backup
  cp "/etc/nginx/sites-available/simpleit" "/etc/nginx/sites-available/simpleit.bak"
...
EOL
  ...
  if [ ! -f "/etc/nginx/sites-enabled/simpleit" ]; then
...
# FIX 8: Fix any replitAuth.ts issues if present
if [ -f "$INSTALL_DIR/server/replitAuth.ts" ]; then
  status_message "Fixing Replit authentication settings"
  # Create a backup
...
# FIX 9: Create diagnostic test page for login testing
status_message "Creating diagnostic test page"
...
# FIX 10: Make sure Express serves static files from public directory
if [ -f "$INSTALL_DIR/server/index.ts" ]; then
...
# FIX 11: Rebuild the application
status_message "Rebuilding the application"
cd "$INSTALL_DIR"
su - $APP_USER -c "cd $INSTALL_DIR && npm run build"

# FIX 12: Create public directory in dist
...
status_message "SimpleIT Master Fix Complete!"
echo ""
...
echo ""
echo "You can now access your SimpleIT installation at: http://your-server-ip/"
echo "Login with username 'admin' and password 'admin123'"
echo ""
echo "For testing authentication specifically, visit: http://your-server-ip/login-test.html"
echo ""
echo "If you encounter any issues:"
echo "- Check logs with: sudo journalctl -u simpleit -f"
